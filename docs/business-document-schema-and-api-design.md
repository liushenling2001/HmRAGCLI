...
注意：本章的知识单元抽取结果是检索和问答的基础事实输入，不等同于知识图谱实体。文档标题、原始文件名、导入批次号和 chunk 标题默认是来源元数据，不应直接提升为业务实体。若后续启用知识图谱增强层，应先经过实体候选过滤、factKind 分类、状态融合和演化构建，详见 `docs/knowledge-graph-evolution-upgrade-plan.md`。
...
4. 对数值、单位、时间必须做归一化
5. 对低置信度结果允许进入待复核队列
6. 文档标题、文件名和导入编号只能作为 `source metadata`，不能默认作为业务实体或关系对象
7. 名称、时间、编号、金额等值型事实应优先作为属性事实；组织、人员、系统、项目、流程等可独立对象才进入实体关系
8. 对规章制度、讲话稿、报告和项目材料应采用不同抽取策略，不能强行都抽成同一种实体状态模型
